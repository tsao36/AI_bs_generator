...
set "LOGFILE=%LOG_DIR%\task_log_dashboard_once_2021_%WEEK_TAG%.txt"
if exist "%SCRIPT_DIR%\task_log_dashboard_once_2021.txt" del /q "%SCRIPT_DIR%\task_log_dashboard_once_2021.txt"
set "PYTHON_EXE="
for %%P in (
...
rem One-time backfill for year 2021 only.
rem NOTE: pipeline treats a single numeric year as rolling refresh; duplicate year forces exact-year mode.
set "PY_ARGS=--created-year 2021,2021 --hsd-limit 500 --hsd-owner-filter yaochien,timdaway --run-option 0 --db-append --no-menu"
...
>>"%LOGFILE%" echo ============================================================
>>"%LOGFILE%" echo [START] One-time 2021 backfill started at: %START_TIMESTAMP%
...
if %EXIT_CODE% EQU 0 (
  >>"%LOGFILE%" echo [SUCCESS] One-time 2021 backfill completed successfully with exit code: %EXIT_CODE%
  >>"%LOGFILE%" echo [SUCCESS] Data update finished at: %END_TIMESTAMP%
) else (
  >>"%LOGFILE%" echo [ERROR] One-time 2021 backfill failed with exit code: %EXIT_CODE%
