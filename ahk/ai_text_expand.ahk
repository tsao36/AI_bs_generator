...
global SuppressNextRightClick := false
...
RButton::
{
    ...
    global SuppressNextRightClick

    ; Ignore the synthetic right-click that we send ourselves to open the native
    ; context menu, otherwise the script can re-enter and loop while it is still
    ; trying to capture the same selection.
    ...
    if isAiMenuRequest {
        ...
        ; Measure the native context menu box before dismissing it, so the AI
        ; menu can be placed beside it instead of underneath it.
        nativeMenuLeft := 0
        nativeMenuRight := 0
        try {
        ...
        DismissNativeContextMenu()
        ShowAiContextMenu(mouseX, mouseY, SelectedText, nativeMenuLeft, nativeMenuRight)
        return
    }
...
DismissNativeContextMenu()
{
    ; Keep pressing Esc until the shell popup menu (#32768) is really gone,
    ; otherwise it stays on top and hides the AI menu.
    loop 10 {
...
ShowAiContextMenu(mouseX, mouseY, selectedText, nativeMenuLeft := 0, nativeMenuRight := 0)
{
    ...
    aiMenu := Menu()
    itemCount := 0
    longestLabel := 34  ; "Expand with Local AI - 5 sentences"
    ...
    menuWidth := longestLabel * 8 + 70
    menuHeight := itemCount * 23 + 16
    GetWorkAreaForPoint(showX, showY, &areaLeft, &areaTop, &areaRight, &areaBottom)

    ; Prefer the right side of where the native menu was; if it would run off
    ; the monitor, try the left side, then fall back to clamping on screen.
    candidateX := (nativeMenuRight > showX + 10) ? nativeMenuRight + 8 : showX + 24
    if (candidateX + menuWidth > areaRight) {
        leftAnchor := (nativeMenuLeft > 0) ? nativeMenuLeft : showX
        candidateX := leftAnchor - menuWidth - 8
    }
    showX := Min(Max(candidateX, areaLeft), areaRight - menuWidth)
    showY := Min(Max(showY, areaTop), areaBottom - menuHeight)
